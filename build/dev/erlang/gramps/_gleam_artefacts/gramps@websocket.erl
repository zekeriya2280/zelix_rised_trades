...
-export([decode_frame/2, apply_mask/2, apply_deflate/2, encode_text_frame/3, encode_binary_frame/3, encode_close_frame/2, encode_ping_frame/2, encode_pong_frame/2, encode_continuation_frame/3, apply_inflate/2, decode_many_frames/3, aggregate_frames/3, make_client_key/0, parse_websocket_key/1, has_deflate/1, get_context_takeovers/1]).
...
-type close_reason() :: not_provided | {normal, bitstring()} | {going_away, bitstring()} | {protocol_error, bitstring()} | {unexpected_data_type, bitstring()} | {inconsistent_data_type, bitstring()} | {policy_violation, bitstring()} | {message_too_big, bitstring()} | {missing_extensions, bitstring()} | {unexpected_condition, bitstring()} | {custom_close_reason, integer(), bitstring()}.

-type control_frame() :: {close_frame, close_reason()} | {ping_frame, bitstring()} | {pong_frame, bitstring()}.

-type frame() :: {data, data_frame()} | {control, control_frame()} | {continuation, integer(), bitstring()}.
                ...
                [One, Two, Three, Four] ->
                    Mask_value = case Index rem 4 of
                        ...
                        _ ->
                            erlang:error(#{
                                gleam_error => panic,
                                message => ~"Somehow a value mod 4 is not 0, 1, 2, or 3",
                                file => ~"src\\gramps\\websocket.gleam",
                                module => ~"gramps/websocket",
                                function => ~"mask_data",
                                line => 66
                            })
                    ...
                    mask_data(Rest, Masks, Index + 1, <<Resp/bitstring, Unmasked/bitstring>>);
...
-spec inflate(boolean(), gleam@option:option(gramps@websocket@compression:context()), bitstring()) -> {ok, bitstring()} | {error, frame_parse_error()}.
...
-spec decode_frame(bitstring(), gleam@option:option(gramps@websocket@compression:context())) -> {ok, {parsed_frame(), bitstring()}} | {error, frame_parse_error()}.
decode_frame(Message, Context) ->
    case Message of
        <<Complete:1, Compressed:1, _:2, Opcode:4/integer, Masked:1, Payload_length:7/integer, Rest/bitstring>> ->
            Compressed@1 = Compressed =:= 1,
            Masked@1 = Masked =:= 1,
            gleam@bool:guard(Compressed@1 andalso gleam@option:is_none(Context), {error, invalid_frame}, fun() ->
                ...
                Maybe_pair = case {Masked@1, Rest} of
                    {true, <<Length:Payload_size/integer, Mask1:1/binary, Mask2:1/binary, Mask3:1/binary, Mask4:1/binary, Rest@1/bitstring>>} ->
                        ...
                        case Rest@1 of
                            <<Payload:Payload_byte_size/binary, Rest@2/bitstring>> ->
                                Data = mask_data(Payload, [Mask1, Mask2, Mask3, Mask4], 0, <<>>),
                                {ok, {Data, Rest@2}};
                    ...
                    {false, <<Length@1:Payload_size/integer, Rest@3/bitstring>>} ->
                        ...
                        case Rest@3 of
                            <<Payload@1:Payload_byte_size@1/binary, Rest@4/bitstring>> ->
                                {ok, {Payload@1, Rest@4}};
                ...
                gleam@result:'try'(Maybe_pair, fun(_use0) ->
                    {Data@1, Rest@5} = _use0,
                    _pipe = case Opcode of
                        0 ->
                            _pipe@1 = Data@1,
                            _pipe@2 = fun(_capture) ->
                                inflate(Compressed@1, Context, _capture)
                            end(_pipe@1),
                            gleam@result:map(_pipe@2, fun(Decompressed) ->
                        ...
                        1 ->
                            ...
                            gleam@result:map(_pipe@4, fun(Decompressed) ->
                                {data, {text_frame, Decompressed}}
                            end);

                        2 ->
                            _pipe@5 = Data@1,
                            _pipe@6 = fun(_capture) ->
                                inflate(Compressed@1, Context, _capture)
                            end(_pipe@5),
                            gleam@result:map(_pipe@6, fun(Decompressed) ->
                                {data, {binary_frame, Decompressed}}
                            end);

                        8 ->
                            case Data@1 of
                                <<1000:16, Rest@6/bitstring>> ->
                                    {ok, {control, {close_frame, {normal, Rest@6}}}};

                                <<1001:16, Rest@7/bitstring>> ->
                                    {ok, {control, {close_frame, {going_away, Rest@7}}}};

                                <<1002:16, Rest@8/bitstring>> ->
                                    {ok, {control, {close_frame, {protocol_error, Rest@8}}}};

                                <<1003:16, Rest@9/bitstring>> ->
                                    {ok, {control, {close_frame, {unexpected_data_type, Rest@9}}}};

                                <<1007:16, Rest@10/bitstring>> ->
                                    {ok, {control, {close_frame, {inconsistent_data_type, Rest@10}}}};

                                <<1008:16, Rest@11/bitstring>> ->
                                    {ok, {control, {close_frame, {policy_violation, Rest@11}}}};

                                <<1009:16, Rest@12/bitstring>> ->
                                    {ok, {control, {close_frame, {message_too_big, Rest@12}}}};

                                <<1010:16, Rest@13/bitstring>> ->
                                    {ok, {control, {close_frame, {missing_extensions, Rest@13}}}};

                                <<1011:16, Rest@14/bitstring>> ->
                                    {ok, {control, {close_frame, {unexpected_condition, Rest@14}}}};

                                <<Code:16, Rest@15/bitstring>> ->
                                    {ok, {control, {close_frame, {custom_close_reason, Code, Rest@15}}}};

                                _ ->
                                    {ok, {control, {close_frame, not_provided}}}
                            end;
                    ...
                    gleam@result:'try'(_pipe, fun(Frame) ->
                        case Complete of
                            1 ->
...
-spec make_frame(integer(), integer(), bitstring(), compression(), gleam@option:option(bitstring())) -> gleam@bytes_tree:bytes_tree().
make_frame(Opcode, Length, Payload, Compressed, Mask) ->
    ...
    _pipe = <<1:1, Compressed@1:1, 0:2, Opcode:4, Masked:1, Length_section/bitstring, Mask_key/bitstring, Payload/bitstring>>,
...
encode_frame(Frame, Compressed, Mask) ->
    case Frame of
        ...
        {control, {close_frame, Reason}} ->
            ...
            make_frame(8, Payload_length@1, apply_mask(Payload@1, Mask), Compressed, Mask);
...
-spec apply_deflate(bitstring(), gleam@option:option(gramps@websocket@compression:context())) -> bitstring().
...
-spec to_frame(bitstring(), gleam@option:option(gramps@websocket@compression:context()), gleam@option:option(bitstring()), fun((bitstring()) -> JLF), fun((JLF) -> frame())) -> gleam@bytes_tree:bytes_tree().
...
-spec encode_text_frame(binary(), gleam@option:option(gramps@websocket@compression:context()), gleam@option:option(bitstring())) -> gleam@bytes_tree:bytes_tree().
encode_text_frame(Data, Context, Mask) ->
    to_frame(gleam_stdlib:identity(Data), Context, Mask, fun(_value) ->
        {text_frame, _value}
    end, fun(_value@1) ->
        {data, _value@1}
    end).
...
-spec encode_binary_frame(bitstring(), gleam@option:option(gramps@websocket@compression:context()), gleam@option:option(bitstring())) -> gleam@bytes_tree:bytes_tree().
encode_binary_frame(Data, Context, Mask) ->
    to_frame(Data, Context, Mask, fun(_value) ->
        {binary_frame, _value}
    end, fun(_value@1) ->
        {data, _value@1}
    end).
...
encode_ping_frame(Data, Mask) ->
    to_frame(Data, none, Mask, fun(_value) ->
        {ping_frame, _value}
    end, fun(_value@1) ->
        {control, _value@1}
    end).
...
-spec encode_continuation_frame(bitstring(), integer(), gleam@option:option(bitstring())) -> gleam@bytes_tree:bytes_tree().
...
-spec apply_inflate(bitstring(), gleam@option:option(gramps@websocket@compression:context())) -> bitstring().
...
-spec decode_many_frames(bitstring(), gleam@option:option(gramps@websocket@compression:context()), list(parsed_frame())) -> {list(parsed_frame()), bitstring()}.
...
-spec aggregate_frames(list(parsed_frame()), gleam@option:option(frame()), list(frame())) -> {ok, list(frame())} | {error, nil}.
...
has_deflate(Extensions) ->
    gleam@list:any(Extensions, fun(Str) ->
        Str =:= ~"permessage-deflate"
    end).
...
get_context_takeovers(Extensions) ->
    No_client_context_takeover = gleam@list:any(Extensions, fun(Str) ->
        Str =:= ~"client_no_context_takeover"
    end),
    No_server_context_takeover = gleam@list:any(Extensions, fun(Str) ->
