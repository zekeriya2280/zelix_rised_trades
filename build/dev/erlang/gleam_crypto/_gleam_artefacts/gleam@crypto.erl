...
-export([strong_random_bytes/1, digest/1, hash_chunk/2, new_hasher/1, hash/2, hmac/3, secure_compare/2, sign_message/3, verify_signed_message/2]).
...
-doc(~" Generates a specified number of bytes randomly uniform 0..255, and returns
...
-doc(~" Finalizes a streaming hash calculation.
...
-doc(~" Adds data to a streaming digest calculation.
...
-doc(~" Initializes the state for a streaming hash digest calculation.
...
-doc(~" Calculates the HMAC (hash-based message authentication code) for a bit
 string.
...
signing_input(Digest_type, Message) ->
    ...
    erlang:list_to_binary([gleam@bit_array:base64_url_encode(<<Protected/binary>>, false), ~".", gleam@bit_array:base64_url_encode(Message, false)]).

-file("src\\gleam\\crypto.gleam", 132).
-spec sign_message(bitstring(), bitstring(), hash_algorithm()) -> binary().
-doc(~" Sign a message which can later be verified using the `verify_signed_message`
...
sign_message(Message, Secret, Digest_type) ->
    ...
    erlang:list_to_binary([Input, ~".", gleam@bit_array:base64_url_encode(Signature, false)]).

-file("src\\gleam\\crypto.gleam", 163).
-spec verify_signed_message(binary(), bitstring()) -> {ok, bitstring()} | {error, nil}.
-doc(~" Verify a message created by the `sign_message` function.
").
verify_signed_message(Message, Secret) ->
    gleam@result:'try'(case gleam@string:split(Message, ~".") of
        [A, B, C] ->
            {ok, {A, B, C}};
        ...
        Text = erlang:list_to_binary([Protected, ~".", Payload]),
        gleam@result:'try'(gleam@bit_array:base64_url_decode(Payload), fun(Payload@1) ->
            gleam@result:'try'(gleam@bit_array:base64_url_decode(Signature), fun(Signature@1) ->
                gleam@result:'try'(gleam@bit_array:base64_url_decode(Protected), fun(Protected@1) ->
                    gleam@result:'try'(case Protected@1 of
                        ...
                        Challenge = gleam_crypto_ffi:hmac(<<Text/binary>>, Digest_type, Secret),
                        case secure_compare(Challenge, Signature@1) of
                            true ->
                                {ok, Payload@1};
