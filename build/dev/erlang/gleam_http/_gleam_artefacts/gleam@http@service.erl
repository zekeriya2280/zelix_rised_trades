...
-spec map_response_body(
    fun((EQH) -> gleam@http@response:response(EQI)),
    fun((EQI) -> EQK)
) -> fun((EQH) -> gleam@http@response:response(EQK)).
...
-spec prepend_response_header(
    fun((EQM) -> gleam@http@response:response(EQN)),
    binary(),
    binary()
) -> fun((EQM) -> gleam@http@response:response(EQN)).
...
-spec ensure_post(gleam@http@request:request(EQQ)) -> {ok,
        gleam@http@request:request(EQQ)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(EQZ)) -> ERB)) -> fun((gleam@http@request:request(EQZ)) -> ERB).
