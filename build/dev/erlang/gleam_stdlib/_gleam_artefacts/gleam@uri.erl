...
query_pair(Pair) ->
    _pipe = [percent_encode_query(erlang:element(1, Pair)), ~"=", percent_encode_query(erlang:element(2, Pair))],
...
query_to_string(Query) ->
    ...
    _pipe@2 = gleam@list:intersperse(_pipe@1, gleam_stdlib:identity(~"&")),
...
to_string(Uri) ->
    ...
    Parts@3 = case {erlang:element(4, Uri), gleam_stdlib:string_starts_with(erlang:element(6, Uri), ~"/")} of
        {{some, Host}, false} when Host =/= ~"" ->
            [~"/" | Parts@2];
    ...
    Parts@4 = case {erlang:element(4, Uri), erlang:element(5, Uri)} of
        {{some, _}, {some, Port}} ->
            [~":", erlang:integer_to_binary(Port) | Parts@3];

        {_, _} ->
            Parts@3
    end,
    Parts@5 = case {erlang:element(2, Uri), erlang:element(3, Uri), erlang:element(4, Uri)} of
...
origin(Uri) ->
    ...
    case {Host, Scheme} of
        {{some, H}, {some, ~"https"}} when Port =:= {some, 443} ->
            {ok, erlang:list_to_binary([~"https://", H])};

        {{some, H@1}, {some, ~"http"}} when Port =:= {some, 80} ->
            {ok, erlang:list_to_binary([~"http://", H@1])};

        {{some, H@2}, {some, S}} when (S =:= ~"http") orelse (S =:= ~"https") ->
            case Port of
                {some, P} ->
                    {ok, erlang:list_to_binary([S, ~"://", H@2, ~":", erlang:integer_to_binary(P)])};

                none ->
                    {ok, erlang:list_to_binary([S, ~"://", H@2])}
            end;
