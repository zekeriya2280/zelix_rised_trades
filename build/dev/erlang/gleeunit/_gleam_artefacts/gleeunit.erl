...
-type eunit_option() :: verbose | no_tty | {report, {report_module_name(), list(gleeunit_progress_option())}} | {scale_timeouts, integer()}.

-file("src\\gleeunit.gleam", 42).
-spec gleam_to_erlang_module_name(binary()) -> binary().
gleam_to_erlang_module_name(Path) ->
    case gleam_stdlib:string_ends_with(Path, ~".gleam") of
        true ->
            _pipe = Path,
            _pipe@1 = gleam@string:replace(_pipe, ~".gleam", ~""),
        ...
        false ->
            ...
            gleam@string:replace(_pipe@5, ~".erl", ~"")
    end.
...
do_main() ->
    ...
    Result = begin
        _pipe = gleeunit_ffi:find_files(~"**/*.{erl,gleam}", ~"test"),
        _pipe@1 = gleam@list:map(_pipe, fun gleam_to_erlang_module_name/1),
        _pipe@2 = gleam@list:map(_pipe@1, fun(_capture) ->
            erlang:binary_to_atom(_capture, utf8)
        end),
...
-spec main() -> nil.
-doc(~" Find and run all test functions for the current project using Erlang's EUnit
