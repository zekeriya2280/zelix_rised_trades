...
-type tcp_option() :: {backlog, integer()} | {nodelay, boolean()} | {linger, {boolean(), integer()}} | {send_timeout, integer()} | {send_timeout_close, boolean()} | {reuseaddr, boolean()} | {active_mode, active_state()} | {mode, socket_mode()} | {cert_key_config, tls_certs()} | {alpn_preferred_protocols, list(binary())} | ipv6 | {buffer, integer()} | {ip, interface()}.
...
-spec merge_type_list(list(GYV), list(GYV)) -> list(GYV).
...
merge_with_defaults(Options) ->
    glisten_ffi:merge_type_list([{backlog, 1024}, {nodelay, true}, {send_timeout, 30000}, {send_timeout_close, true}, {reuseaddr, true}, {mode, binary}, {active_mode, passive}], Options).
