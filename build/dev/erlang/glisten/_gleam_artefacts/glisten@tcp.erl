...
-file("src\\glisten\\tcp.gleam", 49).
-spec shutdown(glisten@socket:socket()) -> {ok, nil} | {error, glisten@socket:socket_reason()}.
shutdown(Socket) ->
    glisten_tcp_ffi:shutdown(Socket, erlang:binary_to_atom(~"write")).

-file("src\\glisten\\tcp.gleam", 60).
...
set_opts(Socket, Opts) ->
    ...
    fun(_capture) ->
        glisten_tcp_ffi:set_opts(Socket, _capture)
    end(_pipe@1).

-file("src\\glisten\\tcp.gleam", 70).
