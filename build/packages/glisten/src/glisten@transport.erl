...
-file("src/glisten/transport.gleam", 207).
...
set_buffer_size(Transport, Socket) ->
    ...
    gleam@result:'try'(
        _pipe@3,
        fun(Value@1) ->
            _pipe@4 = set_opts(Transport, Socket, [{buffer, Value@1}]),
            gleam@result:map_error(_pipe@4, fun(_) -> nil end)
        end
    ).

-file("src/glisten/transport.gleam", 224).
