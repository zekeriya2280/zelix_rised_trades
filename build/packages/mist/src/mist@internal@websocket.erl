...
-type valid_message(LIW) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, LIW}.

-type websocket_message(LIX) :: {valid, valid_message(LIX)} | invalid.
...
-type handler_message(LIY) :: {internal, gramps@websocket:frame()} | {user, LIY}.

-type websocket_state(LIZ) :: {websocket_state,
        bitstring(),
        LIZ,
...
?DOC(false).
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(LKD))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(LKD))).
...
-spec apply_frames(
    ...
    fun((LJT, handler_message(LJU), websocket_connection()) -> mist@internal@next:next(LJT, LJU)),
    websocket_connection(),
    mist@internal@next:next(LJT, websocket_message(LJU)),
    fun((LJT) -> nil)
) -> mist@internal@next:next(LJT, websocket_message(LJU)).
apply_frames(Frames, Handler, Connection, Next, On_close) ->
    case {Frames, Next} of
        ...
        {[{control, {ping_frame, Payload}} | Rest],
            {continue, State@1, _} = Continue} ->
...
-spec initialize_connection(
    fun((websocket_connection()) -> {LJI,
        gleam@option:option(gleam@erlang@process:selector(LJJ))}),
    fun((LJI) -> nil),
    fun((LJI, handler_message(LJJ), websocket_connection()) -> mist@internal@next:next(LJI, LJJ)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
