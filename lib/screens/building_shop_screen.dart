...
class _BuildingShopScreenState extends State<BuildingShopScreen> {
  ...
  static const List<_BuildingInfo> _buildings = [
    _BuildingInfo(
      name: 'Forest', emoji: '🌲',
      description: 'Produces 5 Wood every 30s', baseCost: 5000, type: 'factory',
    ),
    _BuildingInfo(
      name: 'Field', emoji: '🌾',
      description: 'Produces crops over time', baseCost: 8000, type: 'factory',
    ),
    _BuildingInfo(
      name: 'Lumber Mill', emoji: '🪚',
      description: 'Converts 10 Wood → 5 Lumber', baseCost: 15000, type: 'factory',
    ),
    _BuildingInfo(
      name: 'Furniture Factory', emoji: '🪑',
      description: 'Converts 10 Lumber → 2 Furniture', baseCost: 30000, type: 'factory',
    ),
    _BuildingInfo(
  ...
  FactoryType? _getFactoryType(String name) {
    switch (name) {
      case 'Forest': return FactoryType.forest;
      case 'Lumber Mill': return FactoryType.lumberMill;
      case 'Furniture Factory': return FactoryType.furnitureFactory;
      default: return null;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Building Shop', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.brown, fontSize: 22, letterSpacing: 1.5, fontStyle: FontStyle.italic)),
        actions: [
          IconButton(
          ...
          IconButton(
            ...
            onPressed: () => Navigator.of(context).pushReplacement(
              MaterialPageRoute(builder: (context) => const FactoryScreen(warehouseId: "w1")),
            ),
          ),
          IconButton(
            icon: const Icon(Icons.warehouse, color: Colors.black54),
            onPressed: () => Navigator.of(context).pushReplacement(
              MaterialPageRoute(builder: (context) => const WarehouseScreen(warehouseId: "w1")),
            ),
          ),
          IconButton(
        ...
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              gradient: LinearGradient(colors: [Colors.brown[700]!, Colors.brown[500]!]),
              boxShadow: [BoxShadow(color: Colors.brown.withValues(alpha: 0.3), blurRadius: 8, offset: const Offset(0, 4))],
            ),
            child: Row(
              ...
              children: [
                ...
                Text('¥ $money', style: const TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: Colors.white, letterSpacing: 2)),
              ],
            ),
          ),
          ...
          Padding(
            ...
            child: Text('Select a building to purchase:', style: TextStyle(fontSize: 16, color: Colors.brown[600], fontStyle: FontStyle.italic)),
          ),
          // Building list
          Expanded(
  ...
  Widget _buildBottomSummary() {
    ...
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(color: Colors.brown[50], border: Border(top: BorderSide(color: Colors.brown[200]!))),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Your Buildings', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.brown[700])),
          const SizedBox(height: 8),
          Wrap(
            spacing: 16, runSpacing: 4,
            children: owned.map((b) => Text('${b.emoji} ${b.name}: ${b.count}', style: TextStyle(fontSize: 14, color: Colors.brown[600]))).toList(),
          ),
          if (owned.isEmpty)
            Text('No buildings yet. Buy one above!', style: TextStyle(fontStyle: FontStyle.italic, color: Colors.grey[500])),
        ],
      ),
    );
  }
