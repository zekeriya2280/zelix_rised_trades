...
pub fn handle(request: request.Request(mist.Connection), world: process.Subject(game_server.Message)) -> response.Response(mist.ResponseData) {
  case request.path {
    "/ws" -> mist.websocket(
      request: request,
      handler: handle_ws,
      on_init: fn(_connection) { #(WsState(world, option.None), option.None) },
      on_close: fn(state) { disconnect_player(state) },
    )
    "/health" -> text(200, "ok")
    "/auth/login" -> auth_response(request, "login")
    "/auth/register" -> auth_response(request, "register")
    _ -> text(404, "not found")
  }
}

fn handle_ws(state: WsState, message: mist.WebsocketMessage(Nil), connection: mist.WebsocketConnection) -> mist.Next(WsState, Nil) {
  case message {
    mist.Text(text) -> {
      case messages.decode_client(text) {
        ...
        Ok(messages.SetFactoryProduct(id, product)) -> handle_set_factory_product(state, connection, id, product)
        Error(message) -> send_and_continue(state, connection, messages.ServerError(message))
      }
    }
      ...
      False -> case auth.verify_identity(token) {
        ...
        Ok(#(uid, nickname)) -> {
          ...
          case process.receive(reply, within: 1000) {
            ...
            Ok(Error(message)) -> send_and_continue(state, connection, messages.ServerError(message))
...
fn handle_snapshot(state: WsState, connection: mist.WebsocketConnection) -> mist.Next(WsState, Nil) {
  case state.player_id {
    option.None -> send_and_continue(state, connection, messages.ServerError("Authentication required."))
    option.Some(_) -> case send_snapshot_for(state, connection) {
...
fn handle_build_bank(state: WsState, connection: mist.WebsocketConnection, x: Float, y: Float) -> mist.Next(WsState, Nil) {
  case state.player_id {
    option.None -> send_and_continue(state, connection, messages.ServerError("Authentication required."))
    option.Some(player_id) -> {
      let reply = process.new_subject()
      process.send(state.world, game_server.BuildBank(player_id, x, y, reply))
      send_command_result(state, connection, reply)
    }
  }
}

fn handle_build_structure(state: WsState, connection: mist.WebsocketConnection, building: messages.BuildingType, x: Float, y: Float) -> mist.Next(WsState, Nil) {
  case state.player_id {
    option.None -> send_and_continue(state, connection, messages.ServerError("Authentication required."))
    option.Some(player_id) -> {
      let reply = process.new_subject()
      process.send(state.world, game_server.BuildStructure(player_id, building, x, y, reply))
      send_command_result(state, connection, reply)
    }
  }
}

fn handle_spawn_vehicle(state: WsState, connection: mist.WebsocketConnection, x: Float, y: Float, tx: Float, ty: Float, speed: Float) -> mist.Next(WsState, Nil) {
  case state.player_id {
    option.None -> send_and_continue(state, connection, messages.ServerError("Authentication required."))
    option.Some(player_id) -> {
      let reply = process.new_subject()
      process.send(state.world, game_server.SpawnVehicle(player_id, x, y, tx, ty, speed, reply))
      send_command_result(state, connection, reply)
    }
  }
}

fn handle_set_factory_product(state: WsState, connection: mist.WebsocketConnection, id: Int, product: messages.ProductType) -> mist.Next(WsState, Nil) {
  case state.player_id {
    option.None -> send_and_continue(state, connection, messages.ServerError("Authentication required."))
    option.Some(player_id) -> {
      let reply = process.new_subject()
      process.send(state.world, game_server.SetFactoryProduct(player_id, id, product, reply))
      send_command_result(state, connection, reply)
    }
  }
}

fn send_command_result(state: WsState, connection: mist.WebsocketConnection, reply: process.Subject(Result(Nil, String))) -> mist.Next(WsState, Nil) {
  case process.receive(reply, within: 1000) {
...
fn send_snapshot_for(state: WsState, connection: mist.WebsocketConnection) -> Result(Nil, String) {
  case state.player_id {
    option.None -> Error("Authentication required.")
    option.Some(player_id) -> {
      let reply = process.new_subject()
      process.send(state.world, game_server.GetSnapshot(player_id, reply))
      case process.receive(reply, within: 1000) {
        Ok(Ok(snapshot)) -> {
          case mist.send_text_frame(connection, snapshot) {
            Ok(Nil) -> Ok(Nil)
            Error(_) -> Error("Failed to send snapshot.")
          }
        }
        Ok(Error(message)) -> Error(message)
        Error(Nil) -> Error("World timeout.")
      }
    }
  }
}

fn send_and_continue(state: WsState, connection: mist.WebsocketConnection, message: messages.ServerMessage) -> mist.Next(WsState, Nil) {
  let _ = ignore_send(connection, message)
  mist.continue(state)
}

fn ignore_send(connection: mist.WebsocketConnection, message: messages.ServerMessage) {
  let _ = mist.send_text_frame(connection, messages.encode_server(message))
}

fn disconnect_player(state: WsState) {
  case state.player_id {
    option.Some(player_id) -> process.send(state.world, game_server.LeavePlayer(player_id))
    option.None -> Nil
  }
}
...
fn auth_response(request: request.Request(mist.Connection), action: String) -> response.Response(mist.ResponseData) {
  case request.method {
    http.Post -> case mist.read_body(request, 64 * 1024) {
      Ok(body_request) -> {
        let result = auth.handle(action, body_request.body)
        ...
        let status = case result.ok { True -> 200 False -> 401 }
        response.new(status)
        |> response.set_header("content-type", "application/json")
        |> response.set_body(mist.Bytes(bytes_tree.from_string(body)))
      }
      Error(_) -> text(400, "invalid request body")
    }
